...
export function ReceivePanel({ token }: Props) {
  ...
  if (phase.kind === "loading" || phase.kind === "decrypting") {
    return (
      <div className="flex flex-col items-center gap-3 py-16 text-zinc-600">
        <motion.div
          className="h-10 w-10 rounded-full border-2 border-zinc-200 border-t-blue-600"
          animate={{ rotate: 360 }}
    ...
    return (
      ...
      >
        ...
        <input
          ...
          className="w-full rounded-xl border border-zinc-200 px-4 py-3 text-sm outline-none ring-blue-500/30 focus:border-blue-400 focus:ring-2"
  ...
  if (phase.kind === "error") {
    return (
      <motion.div
        ...
        className="max-w-lg rounded-2xl border border-red-200 bg-red-50 px-5 py-4 text-sm text-red-900"
      >
        {phase.message}
      </motion.div>
    );
  ...
  return (
    ...
    >
      ...
      <pre className="max-h-[min(24rem,50vh)] overflow-auto rounded-2xl border border-zinc-200 bg-zinc-50 p-4 text-left text-xs leading-relaxed text-zinc-900">
        {phase.plaintext}
      </pre>
      <div className="flex flex-wrap gap-3">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          onClick={downloadEnv}
          className="rounded-xl border border-zinc-300 bg-white px-5 py-2.5 text-sm font-medium text-zinc-800 shadow-sm hover:bg-zinc-50"
        >
          Download .env
        </motion.button>
      </div>
